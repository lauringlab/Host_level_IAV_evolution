...
beta_Nb<-trans_fit(subset(trans_freq.comp,freq1<0.5),
                   l=NULL,Nb_max=200, model="BetaBin-straight",
                 threshold=0.02,acc=accuracy_stringent,
                 pair_id)
  

beta_Nb_sum<- beta_Nb %>% group_by(pair_id) %>% do(straight_sum(.,200))
...
snv_data<- trans_freq.comp %>% group_by(pair_id) %>%
...
# Adding the ages. 
ages<- read_csv("../data/reference/HIVE_ages_by_season.csv",col_types = cols(STUDY_ID=col_character(),
                                                                            DOB=col_date()))
beta_t<-left_join(beta_t,select(ages,STUDY_ID,DOB),by=c("ENROLLID1"="STUDY_ID")) %>%
  mutate(Donor_age=estimated_transmission_date-DOB)
write.csv(beta_t,"../data/processed/secondary/beta_bottlenecks_by_pair.csv")
```
