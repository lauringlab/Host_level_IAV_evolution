...
beta_Nb<-trans_fit(subset(trans_freq.comp,freq1<0.5),
                   l=NULL,Nb_max=500, model="BetaBin-straight",
                 threshold=0.02,acc=accuracy_stringent,
                 pair_id)
  

beta_Nb_sum<- beta_Nb %>% group_by(pair_id) %>% do(straight_sum(.,500))
...
ages<- read_csv("../data/reference/HIVE_ages_by_season.csv",
  ...
  mutate(DOB = as.Date(parse_date_time(DOB,c("db!y","%m/%d/%Y"))))
  

beta_t<-left_join(beta_t,select(ages,STUDY_ID,AGEYR),by=c("ENROLLID1"="STUDY_ID")) %>% #mutate(Donor_age=as.numeric((estimated_transmission_date-DOB)/365.2425))%>% select(-DOB) %>%
  
  mutate(Donor_age=round(AGEYR,1)) %>% select(-AGEYR) %>%
  left_join(.,select(ages,STUDY_ID,AGEYR),by=c("ENROLLID2"="STUDY_ID")) %>%
  #mutate(Recipient_age=as.numeric((estimated_transmission_date-DOB)/365.2425))
  mutate(Recipient_age=round(AGEYR,1)) %>% select(-AGEYR)
out_beta_t<-beta_t %>% ungroup() %>%
  select(Nb,Subtype=pcr_result,donor_sample,recipient_sample,estimated_transmission_date,Donor_age,Recipient_age,minority_isnv,transmitted_minority_isnv)
