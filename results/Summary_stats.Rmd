...
```{r}
figure1B_data<-snv_qual_meta %>% select(days.post.symptom.onset=DPI,iSNV)
...
isnv_by_isolate.p<-ggplot(snv_qual_meta,aes(x=iSNV))+geom_histogram(color="white",binwidth = 1)+xlab("iSNV in isolate") +ylab("Isolates")
...
```{r,eval=F}
#write.csv(x = snv_qual_meta,"../data/proccessed/meta_snv_qual.csv")
```
...
require(ggjoy)
outliers<-subset(min.count.sample,iSNV>10)
